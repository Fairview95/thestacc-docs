...
3. **Add a database table** for incoming blogs — see [Database schemas](/developers/webhooks/database-schemas) for Postgres, MySQL, Prisma and MongoDB templates.
4. **Add a receiver route** at, for example, `POST /api/stacc-webhook` — see [Receiver examples](/developers/webhooks/receiver-examples) for Next.js, Express, Astro, FastAPI, Flask and PHP.
...
If anything fails, [Limits & troubleshooting](/developers/webhooks/limits-troubleshooting) covers every error we've seen, and [Publishing Errors & Retries](/content-seo/publishing-errors) explains how theStacc surfaces and recovers from a failed publish.
