...
On **autopilot (scheduled) publishes**, the same payload additionally carries two retry-related fields — `idempotency_key` and `publish_attempt`. See [Idempotency & retries](/docs/developers/webhooks/idempotency) below for exactly when they appear and how to use them.
