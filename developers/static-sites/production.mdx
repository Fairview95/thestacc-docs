...
For the full walkthrough — getting the URL from each platform, GitHub Pages via GitHub Actions, and troubleshooting — see [Deploy Hooks](/developers/deploy-hooks).
...
- Review the [Public Blog API](/developers/public-blog-api) reference for every endpoint, parameter, default, and limit
- Set up [Deploy Hooks](/developers/deploy-hooks) so your site rebuilds automatically when blogs are published
- Add an XML sitemap for SEO using the [sitemap endpoint](/developers/public-blog-api#get-blog-sitemap-data)
