...
For a fully static export, add `output: 'export'` to your `next.config.js` and use `generateStaticParams` as shown above. If you'd rather refresh content on a schedule without redeploying, see the **Incremental Static Regeneration** note under [Build-time error handling](/docs/developers/static-sites/error-handling) below.
