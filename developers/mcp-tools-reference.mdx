...
Before you read this, make sure the MCP server is set up and connected to a project. That is covered in [Agent Keys & MCP Server](/developers/agent-keys-mcp). Each agent key is tied to **one project**, so every tool here works on that single project automatically — the agent never has to choose a project.
...
- **Status drives everything.** Most tools only work when a post is in the right status (for example, you can't publish a post that's still generating). The full map of statuses is in [Blog Status Lifecycle](/content-seo/blog-status-lifecycle). The relevant statuses are called out under each tool below.
...
**Important status behavior:** if you edit the **content** of a post that's sitting in **Pending Review** (it was generated and is awaiting your approval), saving moves it to **Modified — Pending Review**. That's the signal that the draft changed after generation and is waiting on a fresh look. Editing only metadata (title, excerpt, meta fields) does not flip the status. See [Blog Status Lifecycle](/content-seo/blog-status-lifecycle) for what each review status means.
...
- [Agent Keys & MCP Server](/developers/agent-keys-mcp) — how to create an agent key and connect the MCP server.
- [Blog Status Lifecycle](/content-seo/blog-status-lifecycle) — what every blog status means and how posts move between them.
- [Public Blog API](/developers/public-blog-api) — read your published posts as JSON for a static site or custom frontend.
