...
These are the fields you'll see on the **Local SEO \> Reviews** page. New reviews come in marked as "new" so you can spot which ones still need a response. See [Reviews](/local-seo/reviews) for how to read, filter, and respond to them.
...
If retrying doesn't resolve a failure, see [Error Handling & Sync Failures](/local-seo/error-handling-sync-failures) for how to read the error details and what each type of failure means.
...
For a fuller breakdown of error types, what they mean, and how to recover from each one, see [Error Handling & Sync Failures](/local-seo/error-handling-sync-failures).

## Related

- [Reviews](/local-seo/reviews) - reading, filtering, and responding to the reviews a sync imports
- [Error Handling & Sync Failures](/local-seo/error-handling-sync-failures) - understanding sync errors and recovering from failures
- [Local SEO Overview](/local-seo/overview) - the full Local SEO dashboard and what each metric means
